...
const SimpleRegister = ({ onSwitchToLogin }) => {
  ...
  const sendApprovalEmail = async (userData) => {
    try {
      await emailjs.send(
        ...
        'template_g8ee2jz',
        {
          to_email: 'info@voltguard.com.tr',
          to_name: 'Admin',
          from_name: userData.name,
          from_email: userData.email,
          company: userData.company || 'Belirtilmemiş',
          message: `Yeni kullanıcı kaydı! Onay bekliyor.\n\nAd: ${userData.name}\nE-posta: ${userData.email}\nŞirket: ${userData.company || '-'}\n\nOnaylamak için admin paneline gidin.`,
          subject: `Yeni Kullanıcı Kaydı - ${userData.name}`
        },
        '-rEVDm1IKnRaw6jCm'
      );
    } catch (error) {
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md text-center">
          ...
          <p className="text-sm text-gray-500">
            Admin e-posta ile bilgilendirildi. Onaylandığınızda giriş yapabilirsiniz.
          </p>
